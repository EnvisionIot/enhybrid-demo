...
#define NAVIGATIONBAR_IMAGE_WIDTH       [UIScreen mainScreen].bounds.size.width
#define NAVIGATIONBAR_IMAGE_HEIGHT      64
...
UIKIT_EXTERN NSString *const kTXSakuraArgGradientImage;
...
+ (void)setSkinCookie:(NSURL *_Nullable) url {
    NSHTTPCookieStorage* cookieStorage = [NSHTTPCookieStorage sharedHTTPCookieStorage];
    // 设置语言cookie
    NSMutableDictionary *properties =[NSMutableDictionary dictionary];
    NSString *name = [TXSakuraManager getSakuraCurrentName];
    if(name == nil || [name isEqualToString:@"default"] ){
        [properties setValue:@"theme-dark" forKey:NSHTTPCookieValue];
    }else{
        [properties setValue:@"theme-light" forKey:NSHTTPCookieValue];
    }
    [properties setValue:@"theme" forKey:NSHTTPCookieName];
...
+ (NSDictionary *)tx_getObjVectorOperationKV {
    return @{
             kTXSakuraArgGradientImage:@"tx_imageGradientWithPath:",
...
+ (UIImage *)tx_imageGradientWithPath:(NSString *)path {
    NSDictionary *imageGradientObj = [self tx_origDictionaryWithPath:path];
    if([imageGradientObj isKindOfClass:[NSDictionary class]] && [imageGradientObj objectForKey:@"colors"] && [imageGradientObj objectForKey:@"direction"]){
        NSString *direction = [imageGradientObj objectForKey:@"direction"];
        NSArray *colors = [imageGradientObj objectForKey:@"colors"]; //从字符A中分隔成2个元素的数组
        NSMutableArray * cgColors = [[NSMutableArray alloc] init];
        for(int i = 0; i < [colors count]; i++) {
            [cgColors addObject:(id)[UIColor colorWithCSS:colors[i]].CGColor];
        }
        CGSize size = CGSizeMake(NAVIGATIONBAR_IMAGE_WIDTH, NAVIGATIONBAR_IMAGE_HEIGHT);
